#include "StarterImageChannelGadget.h"
#include "Node.h"
#include "Types.h"
#include "log.h"
...
namespace Gadgetron {
    ...
    StarterImageChannelGadget::StarterImageChannelGadget(const Context &context, const GadgetProperties &properties)
        : ChannelGadget(context, properties) {}

    void StarterImageChannelGadget::process(InputChannel<Core::AnyImage> &input, OutputChannel &output) {

        std::map<uint16_t, uint16_t> indices{};

        auto index = [&](auto series) { return indices.count(series) ? indices.at(series) : 1; };
        auto increment = [&](auto series) { indices[series] = index(series) + 1 + ImageIndexOffset; };
    ...
    GADGETRON_GADGET_EXPORT(StarterImageChannelGadget);
}