#include "StarterImagePureGadget.h"
...
namespace Gadgetron {
    ...
    AnyImage StarterImagePureGadget::process_function(AnyImage image) const {
        GINFO_STREAM("Inverting image.")
        return Core::visit([](const auto &image) -> AnyImage { return invert_image(image); }, image);
    }

    GADGETRON_GADGET_EXPORT(StarterImagePureGadget);
}
